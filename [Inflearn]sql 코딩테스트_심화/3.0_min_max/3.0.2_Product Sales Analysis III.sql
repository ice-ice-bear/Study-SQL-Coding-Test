...
제품의 판매된 첫 해에 대한 제품 ID, 연도, 수량 및 가격을 선택하는 SQL 쿼리를 작성하세요.
...
rank() over(partition by product_id order by year) as r_year
from sales)f_year
where r_year = 1;
...
# [PRACTICE2]
SELECT PRODUCT_ID,
MIN(YEAR) MIN_YEAR -- 'for the first year of every product sold'
FROM SALES
GROUP BY PRODUCT_ID;
